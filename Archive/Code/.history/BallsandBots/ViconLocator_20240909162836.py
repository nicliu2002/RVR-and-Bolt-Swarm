...
class ViconLocator:
    ...
    def __init__(self, vicon_server_ip='192.168.68.52', vicon_server_port='801', log_file="vicon_log.txt"):
    ...
    def get_position(self, subject_ind):
        """
        Returns the X and Y coordinates of the specified Vicon object.
        """
        self.vicon_client.GetFrame()
        subject_names = self.vicon_client.GetSubjectNames()
        global_position = self.vicon_client.GetSegmentGlobalTranslation(subject_names[subject_ind], subject_names[subject_ind])
        x_vicon = global_position[0][0] / 10
        y_vicon = global_position[0][1] / 10
        data = f"{subject_names[subject_ind]} , {x_vicon}, {y_vicon}"
        self.log_data(data + "\n")
