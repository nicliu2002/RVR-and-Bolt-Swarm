...
class Boid_BOLT:
    ...
    def calculate_vel(self,boid):
        print("calculating velocity from encoders, units in cm/s")
    ...
    def calculate_heading(self):
        ...
        return(math.degrees((math.atan2((nowX-lastX),(nowY-lastY)))))
    
    def run_boid(self, delay):
        with SpheroEduAPI(self.toy) as boid:
            print(f"running {self.id}")
            first_hdg  = random.randint(0,360)
            self.swarm.add_boid(self.id)
            boid.set_speed(50)
            boid.set_heading(first_hdg)
            time.sleep(2.5)
            boid.set_speed(0)
            time.sleep(delay)
            try:
                for count in range(0, 240):
                    ...
                    combined_speed = math.sqrt(combined_vel[0]**2 + combined_vel[1]**2)
                    ...
                    if target_x > wall_rebound_limit or target_x < -wall_rebound_limit or waypoint_x > wall_rebound_limit or waypoint_x < -wall_rebound_limit:
                    ...
                    if target_y > wall_rebound_limit or target_y < -wall_rebound_limit or waypoint_y > wall_rebound_limit or waypoint_y < -wall_rebound_limit:
...
class Boid_RVR:
    ...
    def get_speed_heading(self):
        ...
        time.sleep(0.25)
        nowX, nowY = self.Locator.get_position(self.id)
        return (math.sqrt((nowX-lastX)**2 + (nowY-lastY)**2)),(math.degrees((math.atan2((nowX-lastX),(nowY-lastY)))))
