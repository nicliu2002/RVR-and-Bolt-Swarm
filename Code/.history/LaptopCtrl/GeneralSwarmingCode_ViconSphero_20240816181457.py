...
class Location:
    ...
    def getPosition(self):
        ...
        global_orientation = client.GetSegmentGlobalRotationEulerXYZ(subject_names[self.subject_ind], segment_names[self.subject_ind])
        ...
        while(True):
            ...
            for toy in toys:
                with SpheroEduAPI(toy) as sphero:
                    ...
                    sphero.set_speed(80) 
                    agentPos = Location(sphero,mas,sunject_ind)
                    xPos , yPos = agentPos.getPosition()
                    agent = Agent(sphero, mas, xPos, yPos)
