...
class Swarm2:
    """This class maintains the positions of each Boid and facilitates agent-agent local interactions"""
    def __init__(self, names, viconInstance):
        
        # find all bolts and establish bluetooth connection, increase timeout if struggling to connect 
        filtered_names = [name for name in names if "rvr" not in name.lower()]
        self.toys = scanner.find_toys(toy_names=filtered_names,timeout=12.0)
    ...
    def get_speed_heading(self, id, api):
        """returns speed and heading succinctly for both classes, uses api and last commands passed"""
        if 'SB' in id:
            speed = api.get_speed()
    ...
    def get_next_toy(self):
        """returns toy object for assingment to new boids"""
    ...
    def weighted_sum_forces(forces, weights):
        """combines all forces with weights to create an output velocity"""
        vec_x = 0
        vec_y = 0
